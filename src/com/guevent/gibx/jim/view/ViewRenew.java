...
public class ViewRenew extends JDialog {
	...
	private JButton btnRenew = new JButton("Renew");
	...
	public ViewRenew(){
		setTitle("Family360 - Renew Member");
		calInception1.setDateFormat(dateFormat);
		calExpiry1.setDateFormat(dateFormat);
		pane.add(new JLabel("COI:"), "right");
		pane.add(txtCOI, "w 150, split");
		pane.add(new JLabel("POLICY:"));
		pane.add(txtPolicy, "w 150");
		pane.add(new JLabel("DM:"));
		pane.add(txtDM, "wrap, w 100");
		...
		pane3.add(btnRenew, "span 2, growx");
		...
		pack();
		setModal(true);
		formRenderer = new FormRenderer(this, "renew");
	...
	public void addListener(ActionListener al, FocusListener fl){
		btnRenew.setActionCommand("renew");
		btnRenew.addActionListener(al);
	...
	public void setMember(Member m){
		txtCOI.setText(m.getPolicy_Number());
		txtAssured1.setText(m.getName_Of_Assured());
		txtAssured2.setText(m.getName_Of_Assured2());
		try {
			calInception1.setValue(Util.dateFormatter.parse(m.getInception_Date()));
		} catch (ParseException e) {
			e.printStackTrace();
		}
		txtInception2.setText(m.getInception_Date2());
		try {
			calExpiry1.setValue(Util.dateFormatter.parse(m.getExpiry_Date()));
		} catch (ParseException e) {
			e.printStackTrace();
		}
		txtExpiry2.setText(m.getExpiry_Date2());
		txtDM.setText(m.getDebit_memo());
		txtBeneficiaries.setText(m.getName_of_Beneficiaries());
		txtDependents1.setText(m.getName_of_Family_Members());
		txtDependents2.setText(m.getName_of_Family_Members2());
		txtAddress.setText(m.getAddress());
		txtReferral.setText(m.getReferral_Fee());
		txtTelephone.setText(m.getTelephone());
		txtPolicy.setText(m.getMICO_Policy());
		txtSumAssured.setText(m.getSum_Assured());
		txtPrincipalName.setText(m.getPrincipal_Name());
		txtBasicPrem.setText(m.getBasic_Premium());
		txtInsuranceLine.setText(m.getInsurance_Line());
		txtDStamps.setText(m.getDoc_Stamps());
		txtNote.setText(m.getNote());
		txtVAT.setText(m.getVat());
		txtOthers.setText(m.getOthers());
		txtCompany.setText(m.getReference());
	}
